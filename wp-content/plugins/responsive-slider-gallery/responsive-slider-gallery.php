...
if ( ! class_exists( 'Responsive_Slider_Gallery' ) ) {
	...
	class Responsive_Slider_Gallery {
		...
		public function set_responsive_slider_shortcode_column_name($defaults) {
			...
			foreach($defaults as $key=>$value) {
				if($key=='date') {  // when we find the date column
				   $new['responsive_slider_shortcode'] = __( 'Shortcode', rsg_txt_dm );  // put the tags column before it
				}    
				$new[$key] = $value;
			}
			return $new;  
		...
		public function custom_responsive_slider_shodrcode_data( $column, $post_id ) {
			switch ( $column ) {
				case 'responsive_slider_shortcode' :
					echo "<input type='text' class='button button-primary' id='responsive-slider-shortcode-$post_id' value='[responsive-slider id=$post_id]' style='font-weight:bold; background-color:#32373C; color:#FFFFFF; text-align:center;' />";
					echo "<input type='button' class='button button-primary' onclick='return RESSLIDERCopyShortcode$post_id();' readonly value='Copy' style='margin-left:4px;' />";
					...
					echo "<script>
						function RESSLIDERCopyShortcode$post_id() {
							var copyText = document.getElementById('responsive-slider-shortcode-$post_id');
							copyText.select();
		...
		public function _rsgallery_menu() {
			...
			$featured_plugin_res_menu = add_submenu_page( 'edit.php?post_type='.RSG_PLUGIN_SLUG, __( 'Featured-Plugin', rsg_txt_dm ), __( 'Featured Plugin', rsg_txt_dm ), 'administrator', 'rsgallery-featured-plugin-page', array( $this, '_rsgallery_featured_plugin_page') );
			$premium_plugin_menu = add_submenu_page( 'edit.php?post_type='.RSG_PLUGIN_SLUG, __( 'Upgrade-Plugin', rsg_txt_dm ), __( 'Upgrade Plugin', rsg_txt_dm ), 'administrator', 'rs-upgrade-plugin-page', array( $this, '_rs_upgrade_plugin_page') );
			$theme_menu    = add_submenu_page( 'edit.php?post_type='.RSG_PLUGIN_SLUG, __( 'Our Theme', rsg_txt_dm ), __( 'Our Theme', rsg_txt_dm ), 'administrator', 'sr-theme-page', array( $this, '_rs_theme_page') );
		}		
		...
		public function upload_multiple_images($post) { 
			...
			<div id="slider-gallery">
				...
				foreach($allslidesetting['slide-ids'] as $id) {
					...
					<li class="slide">
						...
						<!-- Slide Title-->
						<input type="text" name="slide-title[]" id="slide-title[]" placeholder="<?php _e('Slide Title', rsg_txt_dm); ?>" readonly value="<?php echo get_the_title($id); ?>">
						<input type="button" name="remove-slide" id="remove-slide" class="button" value="<?php _e('Delete Slide', rsg_txt_dm); ?>">
					</li>
				<?php } // end of foreach
				} //end of if
				?>
			...
			<br>
			<h1 style="font-size: xx-large;"><em><?php _e('Slider Settings', rsg_txt_dm); ?></em></h1>
			<hr>
			<?php
